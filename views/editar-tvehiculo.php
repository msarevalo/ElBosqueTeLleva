...
$consulta = mysqli_query($con,"SELECT * FROM `tipovehiculo` WHERE IdTipoVehiculo='" . $idVehiculo . "';");
$tvehiculo = mysqli_fetch_array($consulta);
...
<div>
    ...
    <div>
        ...
        <form method="post" action="../back/editarTvehiculo.php" id="crear">
            <label for="nombre" class="titulos">Nombre</label>
            <?php
            $_SESSION['idTvehiculo']=$tvehiculo['IdTipoVehiculo'];
            echo "<input type='text' id='nombre' name='nombre' required placeholder='Nombre' value='" . $tvehiculo['TipoVehiculo'] . "' style='text-transform: capitalize'>";
            ?>
            <br>
            <label for="tipo" class="titulos">Estado</label>
            <select id="estado" name="estado" required">
                <?php
                if ($tvehiculo['Estado']==='1'){
